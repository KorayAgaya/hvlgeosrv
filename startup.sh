...
if [ -z "$UID" ]; then
  echo "No UID provided. Exiting."
  exit 1
fi

if [ -z "$GID" ]; then
  echo "No GID provided. Exiting."
  exit 1
fi
...
# /opt/geoserver_data dizininin sahipliğini yeni kullanıcıya ver
chown -R $USERNAME:$USERNAME /opt/geoserver_data
...
if [ "${HTTPS_ENABLED}" = "true" ]; then
  if [ ! -f "${HTTPS_KEYSTORE_FILE}" ]; then
    echo "ERROR: HTTPS was enabled but keystore file was not mounted to container"
    exit 1
  fi
...
# start the tomcat
exec gosu $USERNAME $CATALINA_HOME/bin/catalina.sh run -Dorg.apache.catalina.connector.RECYCLE_FACADES=true

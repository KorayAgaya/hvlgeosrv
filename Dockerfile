...
ARG WAR_ZIP_URL=https://sourceforge.net/projects/geoserver/files/GeoServer/${GS_VERSION}/geoserver-${GS_VERSION}-war.zip
...
RUN set -eux \
    ...
    && apt-get update \
    && apt-get install -y --no-install-recommends openssl gosu unzip curl locales gettext \
    && apt-get clean \
    && rm -rf /var/cache/apt/* \
